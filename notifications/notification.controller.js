...
class NotificationController {
    constructor(destinyService, notificationService, userService, worldRepository) {
                ...
                return this.destiny.getFieldTestWeapons(character.characterBase.characterId,
                    ...
                    .then(items => {
                        if (items && items.length > 0) {
                            const itemHashes = _.map(items, function (item) {
                                    ...
                                    return Promise.all(itemPromises)
                                        .then(items => {
                                            return this.notifications.sendMessage('The following experimental weapons need testing in the field:\n' +
                                                ...
                                                .then(message => {
                                                    return this.users.createUserMessage(user, message, this.users.actions.Gunsmith);
                                                });
                                        })
    ...
    createNotification(notificationType, user) {
        const subscription = user.notifications.find(n => n.type === subscription);
        // todo check last submission?
        if (subscription && subscription.enabled) {
            if (notificationType === types.FieldTestWeapons) {
    ...
    create(req, res) {
        ...
        const { body: { From: phoneNumber }} = req;

        if (phoneNumber) {
            return this.users.getUserByPhoneNumber(phoneNumber)
                .then(user => function () {
                    this.createNotification(subscription, user);
                })
