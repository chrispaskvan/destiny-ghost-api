...
describe('request', () => {
    ...
    describe('post', () => {
        it('should send JSON body and return data', async () => {
            ...
            global.fetch.mockResolvedValue(
                mockResponse({ body: responseBody, contentType: 'application/json' }),
            );
...
describe('request retry logic', () => {
    ...
    it('exhausts retries and throws on persistent transient errors', async () => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(503, 'error', { contentType: 'text/plain' })),
            );
    ...
    it('does not retry on 400 Bad Request', async () => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(400, 'bad request', { contentType: 'text/plain' })),
            );

        vi.stubGlobal('fetch', fetchMock);

        await expect(get({ url: 'https://example.com/api' })).rejects.toMatchObject({
            status: 400,
        });
        expect(fetchMock).toHaveBeenCalledTimes(1);
    });
    ...
    it('does not retry on 401 Unauthorized', async () => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(401, 'unauthorized', { contentType: 'text/plain' })),
            );
    ...
    it('does not retry on 403 Forbidden', async () => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(403, 'forbidden', { contentType: 'text/plain' })),
            );
    ...
    it('does not retry on 404 Not Found', async () => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(404, 'not found', { contentType: 'text/plain' })),
            );
...
describe('ResponseError.isTransient', () => {
    ...
    it.each([408, 429, 500, 502, 503, 504])('status %i is transient', async status => {
        const fetchMock = vi
            .fn()
            .mockImplementation(() =>
                Promise.resolve(makeResponse(status, 'error', { contentType: 'text/plain' })),
            );
